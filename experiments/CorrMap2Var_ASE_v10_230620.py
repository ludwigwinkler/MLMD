...
def PlotComp_CorrMapPhase_SAVE(Var1, Phase1, Var2, Phase2, String1,String2, NumberBins, xLabel, yLabel, grid, AxisRange=[False], tick_spacing = [False]):
    ...
    if HistLog:
        file_str = dataset_filename_[:-4] + ".CorrMap.HistNumpyArray." + Label1 + '-' + Label2 + "." + ExtraString + '_log.pdf'
    else:
        file_str = dataset_filename_[:-4] + ".CorrMap.HistNumpyArray." + Label1 + '-' + Label2 + "." + ExtraString + '.pdf'
    np.save(file_str, histResults[0])
